            ...
            if self.head_resolvable:
                try:
                    if self.user is not None and self.pw is not None:
                        response = requests.head(fp, verify=self.ssl_check, auth=(self.user, self.pw))
                    else:
                        response = requests.head(fp, verify=self.ssl_check)
                except requests.exceptions.MissingSchema:
                    return False
            else:
                try:
                    if self.user is not None and self.pw is not None:
                        response = requests.get(fp, stream=True, verify=self.ssl_check, auth=(self.user, self.pw))
                    else:
                        response = requests.get(fp, stream = True, verify=self.ssl_check)
                except requests.exceptions.MissingSchema:
                    return False
